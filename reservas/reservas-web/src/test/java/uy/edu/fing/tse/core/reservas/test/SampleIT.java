package uy.edu.fing.tse.core.reservas.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "reservas-ear.ear");

        // Current directory is the root of the "reservas-web" project. Go up one level
        // and enter the "reservas-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../reservas-ejb/target/reservas-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "reservas-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/reservas-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "reservas-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
        ...
        war.addPackage("uy.edu.fing.tse.core.reservas.test");
