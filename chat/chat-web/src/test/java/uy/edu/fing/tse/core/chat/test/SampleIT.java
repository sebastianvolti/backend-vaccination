package uy.edu.fing.tse.core.chat.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "chat-ear.ear");

        // Current directory is the root of the "chat-web" project. Go up one level
        // and enter the "chat-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../chat-ejb/target/chat-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "chat-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/chat-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "chat-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
        ...
        war.addPackage("uy.edu.fing.tse.core.chat.test");
